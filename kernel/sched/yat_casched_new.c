...
int select_task_rq_yat_casched(struct task_struct *p, int task_cpu, int flags)
{
    ...
    if (cpu_online(last_cpu) && cpumask_test_cpu(last_cpu, &p->cpus_mask)) {
        ...
        if (cache_age < YAT_CACHE_HOT_TIME) {
            return last_cpu;  /* 缓存热，优先选择上次的CPU */
        }
    }
...
void enqueue_task_yat_casched(struct rq *rq, struct task_struct *p, int flags)
{
    ...
    printk(KERN_INFO "======select yat_casched yat_casched yat_casched yat_casched======\n");
    ...
    if (list_empty(&p->yat_casched.run_list)) {
        ...
        if (p->yat_casched.last_cpu == 0) {
            p->yat_casched.last_cpu = -1;  /* 标记为第一次运行 */
        }
    }
...
void task_tick_yat_casched(struct rq *rq, struct task_struct *p, int queued)
{
    ...
    /* 简单的时间片轮转，每个任务运行一个时间片后切换 */
    if (p->yat_casched.slice >= YAT_MIN_GRANULARITY) {
        p->yat_casched.slice = 0;
        resched_curr(rq);
    } else {
        p->yat_casched.slice += rq_clock_task(rq) - p->se.exec_start;
    }
}
